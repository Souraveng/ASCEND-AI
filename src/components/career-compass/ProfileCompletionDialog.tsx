...
import { UserPlus, Upload, ArrowRight, Sparkles, HelpCircle, Loader2, ArrowLeft, X, CheckCircle } from 'lucide-react';
...
// --- NEW: Helper components for the linking graphic ---
const GoogleIcon = () => (
...
const AppLogoIcon = () => (
    <div className="font-headline font-bold tracking-tight text-lg">
...
export default function ProfileCompletionDialog() {
  const { user, handleProfileUpdate, signupFormState, setSignupFormState, handleCancelSignUp, handleClearAssessmentState } = useAppContext();
  ...
  const watchedFieldOfInterest = form.watch("fieldOfInterest");
  ...
  const renderPanelFour = () => (
    <div className="space-y-4">
        ...
        <div className="text-center pt-2"><Button variant="link" size="sm" type="button" onClick={() => { handleClearAssessmentState(); setSignUpView('quiz'); }}><HelpCircle className="h-4 w-4 mr-2"/>Not sure? Take a quick quiz</Button></div>
    </div>
  );
  
  const renderQuizResults = () => {
    const field = watchedFieldOfInterest;
    return(
    <div>
      ...
      {isSuggesting ? <div className='text-center'><Loader2 className="h-6 w-6 animate-spin mx-auto" /></div> : (
        <div className="flex flex-col gap-3 items-center">{interestSuggestions.map((role, idx) => (<Button key={idx} type="button" size="lg" variant={field === role ? "default" : "outline"} onClick={() => form.setValue("fieldOfInterest", role, { shouldValidate: true, shouldDirty: true })} className="w-full">{role}</Button>))}</div>
      )}
  ...
  return (
    ...
    >
        ...
        >
            <div className="flex h-16 w-16 items-center justify-center rounded-full border bg-background/50 backdrop-blur-sm p-2 shadow-lg">
                <GoogleIcon />
            </div>
