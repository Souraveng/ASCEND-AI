...
export default function LoginDialog() {
  ...
  const { showLogin, setShowLogin, handleLogin, handleSignUp, loginView, setLoginView, handleSignInOrSignUpWithGoogle, handleLoginWithGoogle, signupFormState, setSignupFormState, clearSignupForm, handleClearAssessmentState } = useAppContext();
  ...
  const renderSignupChoice = () => (
    <motion.div key="choice" initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}}>
      <div className="flex items-center gap-3 mb-2"><UserPlus className="h-5 w-5 text-primary" /><h2 className="text-lg font-headline font-semibold">Join Ascend AI</h2></div>
  ...
  const renderPanelFour = () => (
    <div className="space-y-4">
      ...
      {/* --- CORRECTION 2: The onClick handler now resets the quiz state before showing the quiz --- */}
    ...
    <div>
      ...
      {isSuggesting ? <div className='text-center'><Loader2 className="h-6 w-6 animate-spin mx-auto" /></div> : (
        <div className="flex flex-col gap-3 items-center">
            {interestSuggestions.map((role, idx) => (
                <Button 
                    key={idx} 
                    type="button" 
                    size="lg" 
                    variant={field === role ? "default" : "outline"} 
                    onClick={() => form.setValue("fieldOfInterest", role, { shouldValidate: true, shouldDirty: true })}
                    className="w-full"
                >
                    {role}
                </Button>
      ...
      {field && (
          <div className="flex justify-center mt-4">
              <Button type="button" onClick={() => setSignUpView("form")}>
                  Continue with {field}
              </Button>
          </div>
      )}
